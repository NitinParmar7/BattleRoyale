...
void ABattleRoyaleCharacter::OnRep_HealthChanged() 
{
	OnPlayerHealthUpdated.Broadcast(Health);
	if(Health <= 0)
	{
...
void ABattleRoyaleCharacter::DamagePlayer(float Damage)
{
	if(HasAuthority())
	{
		Health = FMath::Clamp<float>((Health - Damage), 0, 100);
		OnRep_HealthChanged();
		if(Health <= 0)
		{
			Destroy();
